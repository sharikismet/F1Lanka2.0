...
export function HeroBanner({ onShopNow }: { onShopNow: () => void }) {
  ...
  return (
    <section className="relative w-full h-[85vh] lg:h-screen bg-[#0a0a0c] overflow-hidden text-white font-sans selection:bg-[#cba153] selection:text-black">
      <Carousel opts={{ loop: true }} plugins={[plugin.current]} className="w-full h-full">
        <CarouselContent className="h-full">
          {HERO_SLIDES.map((slide, index) => (
            <CarouselItem key={index} className="h-full relative">
              
              {/* Background Image with precise dark gradients */}
              ...
              <img 
                src={slide.image} 
                alt={slide.subtitle}
                className="absolute right-0 top-0 w-full md:w-[85%] h-full object-cover opacity-50 mix-blend-luminosity"
              />

              {/* Right Side Watermark Text (Like Jujutsu Kaisen reference) */}
              <div className="absolute right-10 top-[30%] z-20 text-right pointer-events-none opacity-30 hidden md:block">
              ...
              {/* Main Left Content */}
              <div className="absolute left-4 md:left-16 lg:left-24 top-[25%] z-30 max-w-2xl pr-4">
              ...
              {/* Bottom Status Bar */}
              <div className="absolute bottom-12 left-4 md:left-16 lg:left-24 z-30 flex gap-8 md:gap-20 border-t border-white/10 pt-6 pr-10">
                <div>
