...
export function Leaderboard() {
  ...
  // Drag-to-scroll positioning refs
  ...
  useEffect(() => {
    const fetchF1SeasonData = async () => {
      try {
        setLoading(true);

        // 1. Fetch 2026 Championship Standings
        ...
        // 2. Fetch 2026 Schedule and Classification Results
        const scheduleRes = await fetch('https://api.jolpi.ca/ergast/f1/2026/results.json?limit=1000');
        if (!scheduleRes.ok) throw new Error('Results fetch failed');
        const scheduleData = await scheduleRes.json();
        const completedRaces = scheduleData.MRData.RaceTable.Races;

        // Fetch entire calendar to include future events
        const calendarRes = await fetch('https://api.jolpi.ca/ergast/f1/2026.json');
        if (!calendarRes.ok) throw new Error('Calendar fetch failed');
        ...
        const formattedSchedule = allRaces.map((race: any) => {
          ...
          // Compute status via date checking
          const raceDate = new Date(`${race.date}T${race.time || '00:00:00Z'}`);
          const isCompleted = raceDate < new Date() || !!raceClassifications;
        ...
        // Auto-select the latest completed race initially
        const pastRaces = formattedSchedule.filter((r: any) => r.status === 'completed');
        if (pastRaces.length > 0) {
          setActiveTab(pastRaces[pastRaces.length - 1].id);
        }

        setLoading(false);
      } catch (err) {
        console.error("F1 Rebuild fetch layer exception:", err);
        setError("Telemetry link unavailable. Displaying local grid backup.");
        setLoading(false);
      }
    };
  ...
  const getTeamColor = (teamName: string) => {
    ...
    if (name.includes('sauber') || name.includes('kick')) return '#52E252';
    if (name.includes('haas')) return '#B6BABD';
    if (name.includes('rb') || name.includes('racing bulls')) return '#6692FF';
    return '#505050';
  };

  // Horizontal Drag Interaction Controls
  ...
  if (loading) {
    return (
      <div className="w-full h-64 bg-[#0a0a0c] border border-white/10 rounded-sm flex flex-col items-center justify-center text-[#FF2800]">
        ...
        <p className="font-mono text-xs uppercase tracking-widest animate-pulse">Syncing 2026 Core Protocols...</p>
      </div>
    );
  }
  ...
  if (error && standings.length === 0) {
    return (
      <div className="w-full p-6 bg-[#0a0a0c] border border-red-900 rounded-sm text-center font-mono text-sm text-red-500 uppercase">
        {error}
      </div>
    );
  }
      ...
      <div className="p-4 md:p-6 max-h-[620px] overflow-y-auto custom-scrollbar">
        ...
        {currentRace?.status === 'upcoming' ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-500">
            ...
            <p className="font-mono text-xs uppercase tracking-widest">Grand Prix has not commenced</p>
          </div>
        ) : displayData.length === 0 && activeTab !== 'champ' ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-500">
            <Loader2 className="w-10 h-10 mb-3 opacity-30 animate-spin text-[#FF2800]" />
            <p className="font-mono text-xs uppercase tracking-widest">Awaiting Official Classifications</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 flex items-center gap-4 pl-4 border-l border-white/5">
                    ...
                    {/* Portrait Frame mapped directly to number profile naming */}
                    {!hasImageError ? (
                      <div 
